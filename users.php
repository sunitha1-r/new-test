 ...
 if($amnt > $sql1['amount'])
    {
        ...
        $sql = "UPDATE customers set amount=$newCredit where id=$sender";
        mysqli_query($conn,$sql);



        $newCredit = $sql2['amount'] + $amnt;
        $sql = "UPDATE customers set amount=$newCredit where id=$receiver";
        mysqli_query($conn,$sql);

        $sender = $sql1['name'];
        $receiver = $sql2['name'];
        $sql = "INSERT INTO transfers(`sender`, `receiver`, `amount transferred`) VALUES ('$sender','$receiver','$amnt')";
        $tns=mysqli_query($conn,$sql);
        if($tns){
           echo "<script type='text/javascript'>alert('Transaction Successful. Please check your transactions in the transactions table');
...
<head>
    ...
    <title>Sunitha's Basic banking system</title>
...
<body>
  ...
  <center><img src="logo.png" alt="logo" style="width:150px; height:150px"></center>
  </a>
  <p class="logo-text">Sunitha's Banking system</p>
